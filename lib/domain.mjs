...
export function rankSearch(index, query) {
  ...
  const ranked = index
    .map((item) => {
      ...
      if (item.type === "College" && keywords.split(" ").includes(normalizedQuery)) score += 82;
...
export function collegeFitReasons(college, profile) {
  ...
  if (!reasons.length) reasons.push("has a workbook-backed profile; verify funding, testing, and international requirements next");
  return reasons;
}
